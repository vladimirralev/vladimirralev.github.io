...
class SpeechAnalyzer {
    ...
    async start() {
        ...
        try {
            ...
            if (this.inputSource === 'mic') {
                ...
                const testFiles = ['test1.mp3', 'test2.mp3', 'test3.mp3', 'test4.mp3', 'test5.mp3', 'test6.mp3'];
    ...
    drawSpectrum() {
        ...
        const sliceWidth = (this.spectrumWidth * 2) / (this.historySeconds * this.frameRate);
        const binHeight = (this.spectrumHeight * 2) / maxBin;
        
        // Use image data for manual pixel calculation
        const imageData = this.spectrumCtx.createImageData(this.spectrumWidth * 2, this.spectrumHeight * 2);
        for (let i = 0; i < this.spectrumHistory.length; i++) {
            ...
            for (let j = 0; j < maxBin; j++) {
                ...
                for (let dx = 0; dx < sliceWidth && x + dx < this.spectrumWidth * 2; dx++) {
                    for (let dy = 0; dy < binHeight && y + dy < this.spectrumHeight * 2; dy++) {
                        const pixelIndex = ((y + dy) * (this.spectrumWidth * 2) + (x + dx)) * 4;
        ...
        // Draw formant lines (F1 and F2) on the spectrum for the latest data
        if (this.formantHistory.length > 0) {
            ...
            if (latestFormants.f1 && latestFormants.f2) {
                ...
                this.spectrumCtx.moveTo((this.spectrumWidth * 2) - sliceWidth, f1Y);
                this.spectrumCtx.lineTo(this.spectrumWidth * 2, f1Y);
                this.spectrumCtx.stroke();
                this.spectrumCtx.fillText(`${Math.round(latestFormants.f1)} Hz`, (this.spectrumWidth * 2) - 60, f1Y - 5);
                
                // F2 line
                this.spectrumCtx.beginPath();
                this.spectrumCtx.moveTo((this.spectrumWidth * 2) - sliceWidth, f2Y);
                this.spectrumCtx.lineTo(this.spectrumWidth * 2, f2Y);
                this.spectrumCtx.stroke();
                this.spectrumCtx.fillText(`${Math.round(latestFormants.f2)} Hz`, (this.spectrumWidth * 2) - 60, f2Y - 5);
            }
        }
    ...
    getColorForPower(value) {
        ...
        if (value < 85) return { r: value * 3, g: 0, b: 0 }; // Black to Red (low power)
        if (value < 170) return { r: 255, g: (value - 85) * 3, b: 0 }; // Red to Orange
        return { r: 255, g: (value - 170) * 1.5, b: 0 }; // Orange to Red (high power)
    }
    ...
    drawSpectrumAxes() {
        ...
        this.spectrumCtx.lineTo(30, this.spectrumHeight * 2);
        this.spectrumCtx.stroke();
        const maxFreq = 3500;
        ...
        // X-axis (Time)
        this.spectrumCtx.beginPath();
        this.spectrumCtx.moveTo(0, (this.spectrumHeight * 2) - 30);
        this.spectrumCtx.lineTo(this.spectrumWidth * 2, (this.spectrumHeight * 2) - 30);
        this.spectrumCtx.stroke();
        const timeStep = this.historySeconds / 5;
        for (let i = 0; i <= 5; i++) {
            const x = i * ((this.spectrumWidth * 2) / 5);
            ...
            this.spectrumCtx.beginPath();
            this.spectrumCtx.moveTo(x, (this.spectrumHeight * 2) - 25);
            this.spectrumCtx.lineTo(x, (this.spectrumHeight * 2) - 35);
            this.spectrumCtx.stroke();
        }
    }
